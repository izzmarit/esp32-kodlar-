...
bool InitializeFilesystem() {
    // LittleFS'i başlat (true parametresi ile formatla)
    if (!LittleFS.begin(true)) {
        LOG_ERROR(FS_TAG, "LittleFS başlatılamadı!");
        
        // Formatlamayı dene
        LOG_INFO(FS_TAG, "LittleFS formatlanıyor...");
        if (!LittleFS.format()) {
        ...
        LOG_INFO(FS_TAG, "LittleFS başlatıldı");
    }
