    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i] && clients[i].connected()) {
            ...
            if (clients[i].available()) {
                ...
                // DÜZELTİLMİŞ BÖLÜM - Veri okuma mantığı iyileştirildi
                // En fazla 1000ms bekle veya tam satır oku
                while ((millis() - startTime < 1000) && clients[i].available()) {
                    ...
                    // Kısa bir bekleme ekle - veri paketlerinin tamamlanması için
                    delay(1);
                }
                
                // Veriyi temizle - yalnızca gereksiz boşlukları kaldır, satır sonlarını bırak
                data.trim();
...
bool WifiSendData(const String& data, WiFiClient& targetClient) {
    ...
    if (targetClient && targetClient.connected()) {
        // Veriyi gönder - satır sonu karakteri ekleyerek
 ...
 void SendSensorData(WiFiClient& client) {
     ...
     doc["temperature"] = sensorData.avgTemperature;
