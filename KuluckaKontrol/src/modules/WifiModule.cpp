...
void updateAPMode() {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (!clients[i] || !clients[i].connected()) {
            // Önceki istemci varsa kapatın ve sayacı düzgün ayarlayın
            if (clients[i]) {
                clients[i].stop();
                // Burada clientCount azaltılmıyor çünkü yeni bir istemci atanacak
            }
            ...
            // İstemci sayacını güncelle - önceden bağlı değilse artır
            if (!clientConnected) {
                clientCount++;
            }
