 ...
 */
 ...
 void DisplayShowMain(float temp, float humidity, int day, int totalDays, 
    ...
    if (abs(temp - lastValues.temperature) >= 0.1 || heaterOn != lastValues.heaterOn || isFirstMainDisplay) {
        ...
        // Isıtıcı durum göstergesi - daha iyi konumlandırma
        int indicatorX = tempX + tempWidth + 3;
        int indicatorY = mainAreaTop + 25;
        
        if (heaterOn) {
            tft.fillCircle(indicatorX, indicatorY, 3, ST7735_RED);
        } else {
            tft.fillCircle(indicatorX, indicatorY, 3, ST7735_BLACK);
        }
        
        // Hedef sıcaklık etiketi
        tft.setCursor(5, mainAreaTop + 45); // 40 yerine 45 yaparak biraz aşağı kaydırıyoruz
        tft.setTextSize(1);
    ...
    // Nem değişimini kontrol et ve güncelle - benzer düzenlemeler
    if (abs(humidity - lastValues.humidity) >= 0.5 || humidifierOn != lastValues.humidifierOn || isFirstMainDisplay) {
        ...
        // Yeni nemi çiz - Daha küçük boyut ve doğru hizalama
        char humDisplay[8];
        sprintf(humDisplay, "%.0f%%", humidity);
        
        // Metin genişliğini hesapla ve ortala
        int humWidth = strlen(humDisplay) * 10; // Size 2 için yaklaşık genişlik
        ...
        // Nemlendirici durum göstergesi - daha iyi konumlandırma
        int indicatorX = humX + humWidth + 3;
        int indicatorY = mainAreaTop + 25;
        
        if (humidifierOn) {
            tft.fillCircle(indicatorX, indicatorY, 3, ST7735_BLUE);
        } else {
            tft.fillCircle(indicatorX, indicatorY, 3, ST7735_BLACK);
        }
        
        // Hedef nem etiketi
        tft.setCursor(tft.width() / 2 + 5, mainAreaTop + 45); // 40 yerine 45 yaparak biraz aşağı kaydırıyoruz
        tft.setTextSize(1);
