 ...
 */
 ...
 void DisplayShowMain(float temp, float humidity, int day, int totalDays, 
    ...
    // EKLENEN KOD: Isıtıcı durum göstergesi - kırmızı yuvarlak
    if (heaterOn) {
        // Daha büyük ve belirgin bir kırmızı yuvarlak
        tft.fillCircle(tempX - 10, mainAreaTop + 25, 5, ST7735_RED);
    } else {
        // Isıtıcı kapalıysa yuvarlağı sil
        tft.fillCircle(tempX - 10, mainAreaTop + 25, 5, ST7735_BLACK);
    }
...
if (abs(humidity - lastValues.humidity) >= 0.5 || humidifierOn != lastValues.humidifierOn || isFirstMainDisplay) {
    ...
    // EKLENEN KOD: Nemlendirici durum göstergesi - mavi yuvarlak
    if (humidifierOn) {
        // Daha büyük ve belirgin bir mavi yuvarlak
        tft.fillCircle(humX - 10, mainAreaTop + 25, 5, ST7735_BLUE);
    } else {
        // Nemlendirici kapalıysa yuvarlağı sil
        tft.fillCircle(humX - 10, mainAreaTop + 25, 5, ST7735_BLACK);
    }
