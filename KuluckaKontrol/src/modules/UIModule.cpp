 ...
 */
 ...
 static const unsigned long TRANSITION_DURATION = 20;
...
void UIHandleTransition() {
    if (inTransition) {
        ...
        // Geçiş süresini artıralım
        if (currentTime - transitionStartTime >= 100) {  // 20ms -> 100ms
            inTransition = false;
...
bool handleMotorMenu(JoystickDirection direction) {
    ...
    // Test bitimi için kısa bir bekleme
    delay(2000);
...
void UINavigateToMenu(MenuState menu) {
    ...
    // Geçiş öncesi ekranı temizle
    tft.fillScreen(ST7735_BLACK);
    
    // Başlık çizimi - ekran boş görünmesin
    tft.fillRect(0, 0, tft.width(), 16, ST7735_NAVY);
    tft.setCursor(5, 5);
    tft.setTextSize(1);
    tft.setTextColor(ST7735_WHITE);
    
    // Geçiş başlığını göster
    switch (menu) {
        case MENU_MAIN: tft.println("ANA MENU"); break;
        case MENU_GENERAL_SETTINGS: tft.println("GENEL AYARLAR"); break;
        case MENU_TEMP_HUMIDITY: tft.println("SICAKLIK VE NEM"); break;
        case MENU_MOTOR: tft.println("MOTOR AYARLARI"); break;
        case MENU_PROFILE: tft.println("PROFIL"); break;
        case MENU_GRAPH: tft.println("GRAFIKLER"); break;
        case MENU_POWER_SAVE: tft.println("ENERJI TASARRUF"); break;
        case MENU_TIME_DATE: tft.println("TARIH/SAAT"); break;
        case MENU_ALARM: tft.println("ALARM AYARLARI"); break;
        case MENU_WIFI: tft.println("WIFI AYARLARI"); break;
        case MENU_CALIBRATION: tft.println("KALIBRASYON"); break;
        case MENU_BACKUP: tft.println("YEDEKLEME"); break;
        case MENU_NONE: tft.println("ANA EKRAN"); break;
        default: tft.println("MENU"); break;
    }
    
    // Geçiş bilgisi
    tft.setCursor((tft.width() - 80) / 2, tft.height() / 2);
    tft.println("Yukleniyor...");
}
...
bool waitForButton() {
    ...
    delay(50);
    while (digitalRead(JOYSTICK_BTN) == LOW) {
        esp_task_wdt_reset(); // Eklenen satır
        delay(10);
    }
    delay(50);
