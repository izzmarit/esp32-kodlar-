...
void UIHandleTransition() {
    if (inTransition) {
        ...
        // Transition süresini 20ms'ye düşürelim (100ms'den)
        if (currentTime - transitionStartTime >= 20) {
            inTransition = false;
...
bool handleMotorMenu(JoystickDirection direction) {
    ...
    // DÜZELTME: Test bitimi için bekleme süresi ve güncelleme ekleyelim
    // Motor test süresinin bitmesini bekle
    delay(2000); // Kullanıcıya bildirim gösterildikten sonra 2 saniye bekle
    
    // Ana ekrana dön - bu sayede motor durumu güncellenecek
    UIBackToMain();
    
    // Kısa bir gecikme sonrasında menüye geri dön
    delay(1000);
    UINavigateToMenu(MENU_MOTOR);
    return true;
}
