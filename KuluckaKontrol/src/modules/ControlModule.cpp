...
if (relayState.motor && currentTime >= relayState.motorRunningUntil) {
    ...
    // DÜZELTME: Motor durduğunda lastMotorActivation güncellenirken
    // motorInterval'ı sıfırla fakat çalışma süresini çıkar
    // Bu şekilde toplam aralık = motorInterval olacak (çalışma süresi dahil)
    relayState.lastMotorActivation = currentTime - motorInterval + motorDuration;
