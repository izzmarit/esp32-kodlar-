 ...
 * @version 1.7 - PID menü basitleştirildi ve Android entegrasyonu tamamlandı
 */
...
void handleValueAdjustment(JoystickDirection direction) {
    if (direction == JOYSTICK_PRESS) {
        ...
        if (prevState >= MENU_PID_KP && prevState <= MENU_PID_KD) {
            menuManager.setCurrentState(MENU_CALIBRATION);  // PID parametreleri kalibrasyonda
        }
